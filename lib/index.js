...
function allImageTagAltIsConsistent(url,callback){
	...
	var ret = lodash.forEach(imgs, function (img1) {
		...
		//console.log(img.attribs.alt)
		if (typeof img1Alt == 'undefined' || img1Alt == '') {
			//console.log("FALSE");
			console.log("\t\tMissing: "+img1.attribs.src);
			missCnt += 1;
			//return false;
		}
		return retIn = lodash.forEach(imgs, function(img2) {
			...
			//imgCnt += 1;
			// if the alt text isn't there or is empty, return false
			if (typeof img2Alt == 'undefined' || img2Alt == ''){
				return true;
			}
			// if the alt text doesn't meet the condition, return false
	...
	//console.log("TRUE");
	if(failCnt/2 == 0 && missCnt == 0)
		ret = true;
	else
		ret = false;
	console.log("\tNumber of images = " + imgs.length);
	console.log("\tNumber of images missing alt text = "+missCnt);
	console.log("\tNumber of inconsistencies found = "+failCnt/2);
	console.log("\tAll images are consistent: "+ret+"\n");
	if (ret == true)
	{
		//Acnt += 0.5;
		callback(null, 0.5);
...
function someSectionsHaveLang(url,callback){
	...
	var ret = request.get({url : url, rejectUnauthorized : false}, function(err, resp, html) {
		...
		var langP = $("*");
		var langCnt = 0;
		var ret = lodash.forEach(langP, function (lang) 
		{
			if($(lang).attr('lang') != undefined)
				langCnt++
		});
		var isFound = true;
		if (langCnt > 1)
        ...
        return page.run(url,function (url) {
            ...
            return this.evaluate(function () {
            	...
            	for(var i=0;i< links.length;i++) {
            		...
            		if(lurl.indexOf("http") > -1 && lurl.indexOf('#') == -1)
	...
	var req = request.get({url : url, rejectUnauthorized : false}, function(err, resp, body) {
		if (err)
			...
			if (nav.length < 1)
			{
				useNav = false;
				nav = $('ul');
				//grab ul tags that have navigation roles
				lodash.every(nav, function(navs){
					if ($(navs).attr('role') != undefined && 
					($(navs).attr('role') == 'menu' || $(navs).attr('role') == 'menubar' || $(navs).attr('role') == 'navigation'))
					{
						gnav[gnav.length] = navs;
						//console.log(navs);
					}
					return true;
				});
	...
	var getNav = function(link, cb) {
		...
		var req = request.get({url : link, rejectUnauthorized : false}, function(err, resp, body) {
			if (err)
				...
				if (useNav == false && navsi.length < 1)
				{
					navsi = $('ul');
					//console.log(navsi.toString());
					//console.log(navsi);
					cb(null, lodash.every(navsi, function(nav){
						...
						{
							var nmenu = navsi.toString();
							/*console.log("nmenu: "+nmenu);
							var match = false;
							for(var i = 0; i < gnav.lenght; i++)
							{
								console.log("orig: "+gnav[i].toString());
								if (gnav[i].toString() == nmenu)
								{
									match = true;
									break;
								}	
							}
							// compare to list of navigation ul tags
							if(match)*/
							if( nmenu == menu )
								return false
							else
    ...
    .then(function (page) {
        ...
        return page.run(url,function (url) {
            ...
            return this.evaluate(function () {
            	...
            	for(var i=0;i< links.length;i++) {
            		var lurl = ''+links[i].href;
            		if(lurl.indexOf("http") > -1 && lurl.indexOf('#') == -1 && ((lurl.indexOf(core) == 7 || lurl.indexOf(core) == 8) 
    ...
    .done(function (links) {
    	...
    	async.map(nLinks,getNav, function(err, results) {
    		...
    		else
    		{
    			...
    			console.log("\tSites checked with main page: "+nLinks.length-1);
